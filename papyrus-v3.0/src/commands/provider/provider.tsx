...
export function buildProfileSaveMessage(
  ...
  lines.push('Restart Papyrus to use it.')
...
function buildUsageText(): string {
  ...
  return [
    ...
    '',
    'Choose Auto, Ollama, OpenAI-compatible, Gemini, or Codex, then save a profile for the next Papyrus restart.',
  ].join('\n')
}
...
function ProviderChooser({
  ...
  if (summary.savedProfileLabel !== 'none') {
    options.push({
      ...
      value: 'clear',
      description: 'Remove .papyrus-profile.json and return to normal startup',
    })
  }
  ...
  return (
    ...
    >
      <Box flexDirection="column" gap={1}>
        <Text>
          Save a provider profile for the next Papyrus restart without
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart Papyrus to go back to normal startup.`, {
                display: 'system',
              })
            } else {
